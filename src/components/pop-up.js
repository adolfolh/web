  ...
  return (
    <div className="popup">
        ...
        <ul className="flex justify-between mb-8 text-white">
            ...
            <li><a className="pr-2" target="_blank" rel="noreferrer" href={`https://wa.me/whatsappphonenumber?text=${window.location.href}`}><span className="label"><FontAwesomeIcon icon={faFacebookMessenger} size="2x"/></span></a></li>
        ...
        <h2 className="py-4 mb-8" >Or copy link</h2>
        <div className="field w-full">
            <i className="url-icon uil uil-link"></i>
            <input className="p-2 w-full text-gray-500" type="text" readonly value={window.location.href}/>
            <button onClick={() =>  navigator.clipboard.writeText(`${window.location.href}`)} className="w-full text-center bg-white hover:bg-azure hover:text-white text-black font-bold py-2 px-12 lg:px-8 duration-200 transition-all lg:mb-0 font-accent">Copy</button>
        </div>
  </div>
  );
 }
